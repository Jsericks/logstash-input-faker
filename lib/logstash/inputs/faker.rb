...
class LogStash::Inputs::Faker < LogStash::Inputs::Base
  ...
  def add_splitable_fields(event)
    ...
    @splitable_field_count.times do
      ...
      splitable_field_state = Array(event.get(@splitable_field))
      splitable_field_state << new_fields
      event.set(@splitable_field, splitable_field_state)
      # LogStash::Util::Decorators.add_fields(new_fields.to_hash, event, "inputs/#{self.class.name}")
    end
  end
